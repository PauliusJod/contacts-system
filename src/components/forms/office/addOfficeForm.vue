  ...
  <div class="relative bg-white rounded-sm shadow-xl dark:bg-gray-700">
    <div class="p-8 text-left">
      <div class="flex justify-between mb-10">
        <p class="py-1 text-2xl text-black">Pridėkite naują ofisą:</p>
        <button
      ...
      <div class="grid grid-cols-2 gap-8 py-8">
        <div class="grid grid-cols-1 gap-4">
          <div class="pb-4">
            <p class="text-sm text-gray-800 dark:text-gray-400">Adresas:</p>
            <input
              v-model="addressValue"
              class="border border-sm outline-0 bg-gray-100 text-black text-sm text-normal p-2"
              placeholder="Įveskite ofiso adresą..."
            />
          </div>
          <div class="pb-4">
            <p class="text-sm text-gray-800 dark:text-gray-400">Miestas:</p>
            <input
              v-model="cityValue"
              class="border border-sm outline-0 bg-gray-100 text-black text-sm text-normal p-2"
          ...
          <div class="pb-4">
            <p class="text-sm text-gray-800 dark:text-gray-400">Šalis:</p>
            <input
        ...
        <div>
          <div class="h-fit" v-for="(option, index) in companies">
            <input
              :id="'office' + index"
              type="checkbox"
              value=""
          ...
          <div class="p-4" v-if="addressValue != '' && cityValue != '' && countryValue != ''">
            <button
              :disabled="addressValue == '' || cityValue == '' || countryValue == ''"
              data-modal-hide="popup-modal"
              type="button"
              @click="addItem()"
